param(
    [string]$TestFilter = "Category=FrontendQualityAggregateGuard"
)
...
$selected = @($discovery | Where-Object { $_ -match '^\s+BirkNext\.Web\.Tests\.' }).Count
Write-Host "Selected $selected"
if ($selected -eq 0)
{
    Write-Host "ERROR: Phase 2E aggregate guard selected 0 tests for filter '$TestFilter'." -ForegroundColor Red
    exit 2
}
