param(
    [string]$TestFilter = "Category=FrontendBrowserRuntimeIntegration"
)
...
if ($selected -eq 0) {
    Write-Host "ERROR: Browser Runtime gate selected 0 tests for filter '$TestFilter'." -ForegroundColor Red
    exit 2
}
