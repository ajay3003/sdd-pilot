...
public class SpecificationReviewPageModelBuilder(
    IWorkspaceArtifactStatusService artifactStatus) : ISpecificationReviewPageModelBuilder
{
    public async Task<ReviewPageModel> BuildAsync()
    {
        ...
        if (artifact == null)
        {
            return new ReviewPageModel
            {
                ...
                Description = "Review and analyze specification.md files",
                ReadinessStatus = ReviewStatus.Empty,
                RequiredInputs = new[] { "Specification" }.ToList(),
                Summary = new ReviewSummary
                {
        ...
        var results = new List<ReviewResult>();
        var lines = artifact.Content.Split('\n');
        var requirements = CountRequirements(lines);
        var useCases = CountUseCases(lines);
        var acceptanceCriteria = CountAcceptanceCriteria(lines);
        ...
        results.Add(new ReviewResult
        {
            ...
            Status = ReviewStatus.Ready,
            Severity = "Info",
            Summary = $"Specification defines {requirements} requirements, {useCases} use cases, {acceptanceCriteria} acceptance criteria",
            Details = $"Document contains {lines.Length} lines with {CountSections(lines)} major sections"
        });
        ...
        if (requirements == 0)
        {
            results.Add(new ReviewResult
            {
                Name = "No Requirements Found",
                Category = "Validation",
                Status = ReviewStatus.Warning,
                Severity = "Warning",
                Summary = "Specification contains no identifiable requirements",
                Recommendation = "Add requirement statements using standard notation (REQ-xxx)"
            });
        }
        ...
        if (useCases == 0)
        {
            results.Add(new ReviewResult
            {
                ...
                Status = ReviewStatus.Ready,
                Severity = "Info",
                Summary = "Specification does not document use cases",
                Recommendation = "Consider adding use case descriptions for complex features"
            });
        }
        ...
        return new ReviewPageModel
        {
            ...
            Summary = new ReviewSummary
            {
                StatusMessage = $"Specification loaded with {requirements} requirements",
                TotalResults = results.Count,
                WarningCount = requirements == 0 ? 1 : 0,
                CanRun = true,
                HasAvailableActions = true
            }
        };
    ...
    private static int CountRequirements(string[] lines) => lines.Count(l => l.Contains("REQ-") || l.Contains("requirement") || l.Contains("Requirement"));
    private static int CountUseCases(string[] lines) => lines.Count(l => l.Contains("Use Case") || l.Contains("use case"));
    private static int CountAcceptanceCriteria(string[] lines) => lines.Count(l => l.Contains("Acceptance") || l.Contains("Given") || l.Contains("When") || l.Contains("Then"));
    private static int CountSections(string[] lines) => lines.Count(l => l.StartsWith("## "));
}
