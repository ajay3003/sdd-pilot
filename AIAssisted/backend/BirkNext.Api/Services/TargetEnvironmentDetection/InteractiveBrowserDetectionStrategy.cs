...
internal sealed class InteractiveBrowserDetectionStrategy : ITargetDetectionAuthenticationStrategy
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogInformation($"[DIAG] InteractiveBrowserDetectionStrategy.ContinueDetectionAsync START targetUrl={targetUrl}");
            ...
            _logger.LogInformation($"[DIAG] InteractiveBrowserDetectionStrategy about to call StartAsync");
            var sessionDescriptor = await _sessionManager.StartAsync(sessionRequest, cancellationToken);
            _logger.LogInformation($"[DIAG] InteractiveBrowserDetectionStrategy StartAsync returned status={sessionDescriptor.Status} sessionId={sessionDescriptor.SessionId}");
            ...
            if (sessionDescriptor.Status != AuthenticatedBrowserSessionStatus.BrowserReady)
            {
                ...
                _logger.LogInformation($"[DIAG] InteractiveBrowserDetectionStrategy browser not ready: {sessionDescriptor.Status}");
                return new DetectionContinuationResult
                {
                    AuthenticationSucceeded = false,
