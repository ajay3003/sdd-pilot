...
[Trait("Category", "AuthenticatedReviewPhaseA5RealAcceptance")]
public sealed class AuthenticatedReviewPhaseA5RealAcceptanceTests
{
    private const string Review = "phase-a5-review";
    private const string Profile = "phase-a5-profile";

    [Fact]
    public async Task AuthenticatedReview_BrowserRuntimeThenAccessibility_AssessesBothOnSameSession()
    {
        ...
        await using var original = await manager.AcquireAuthenticationPageLeaseAsync(session.SessionId, Review, Profile, fixture.TargetUrl);
        var page = original.Page; var context = original.Context;
        var targetRequests = fixture.TargetRequestCount;
        ...
        harness.Accessibility.ObservedPage.Should().BeSameAs(page);
        harness.Runtime.ObservedContext.Should().BeSameAs(context);
        harness.Accessibility.ObservedContext.Should().BeSameAs(context);
        context.Pages.Should().ContainSingle().Which.Should().BeSameAs(page);
        context.Browser!.IsConnected.Should().BeTrue();
        fixture.TargetRequestCount.Should().Be(targetRequests);
        (await manager.GetStatusAsync(session.SessionId, Review, Profile))!.Status.Should().Be(AuthenticatedBrowserSessionStatus.Authenticated);
        await using var reacquired = await manager.AcquirePageLeaseAsync(session.SessionId, Review, Profile, fixture.TargetUrl);
        reacquired.Page.Should().BeSameAs(page);
        reacquired.Context.Should().BeSameAs(context);
    }

    [Fact]
    public async Task AuthenticatedReview_RuntimeUnexpectedOrigin_AccessibilityNotInvoked()
    {
        if (!ExternalFrontendQualityTestGate.IsLocalHeadedEnabled) return;
        await using var fixture = await AuthenticatedReviewPhaseA4RealAcceptanceTests.SyntheticFixture.StartAsync();
        await using var manager = CreateManager();
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task AuthenticatedReview_EntraOrMcasInvalidation_AccessibilityNotInvoked(bool mcas)
    {
        if (!ExternalFrontendQualityTestGate.IsLocalHeadedEnabled) return;
        await using var fixture = await AuthenticatedReviewPhaseA4RealAcceptanceTests.SyntheticFixture.StartAsync();
        await using var manager = CreateManager();
        ...
        harness.Orchestrator.AuthenticatedObserver = new CallbackObserver(async () =>
        {
            await lease.Page.GotoAsync(mcas ? $"{fixture.McasOrigin}/notice" : fixture.EntraUrl);
    ...
    [Fact]
    public async Task AuthenticatedReview_SessionExpiresBetweenEngines_AccessibilityNotInvoked()
    {
        if (!ExternalFrontendQualityTestGate.IsLocalHeadedEnabled) return;
        ...
        var session = await ReachAuthenticatedAsync(manager, fixture);
        var harness = CreateHarness(manager, session.SessionId, fixture.TargetUrl);
        harness.Orchestrator.AuthenticatedObserver = new CallbackObserver(() =>
        {
    ...
    [Fact]
    public async Task AuthenticatedReview_PageClosedBetweenEngines_NoAnonymousFallbackOrEvidence()
    {
        if (!ExternalFrontendQualityTestGate.IsLocalHeadedEnabled) return;
        await using var fixture = await AuthenticatedReviewPhaseA4RealAcceptanceTests.SyntheticFixture.StartAsync();
        await using var manager = CreateManager();
        ...
        await using var lease = await manager.AcquireAuthenticationPageLeaseAsync(session.SessionId, Review, Profile, fixture.TargetUrl);
        var harness = CreateHarness(manager, session.SessionId, fixture.TargetUrl);
        harness.Orchestrator.AuthenticatedObserver = new CallbackObserver(() => lease.Page.CloseAsync());
    ...
    [Fact]
    public async Task AuthenticatedReview_SanitizedEvidenceSurvivesCompositeReport()
    {
        if (!ExternalFrontendQualityTestGate.IsLocalHeadedEnabled) return;
        await using var fixture = await AuthenticatedReviewPhaseA4RealAcceptanceTests.SyntheticFixture.StartAsync();
        await using var manager = CreateManager();
        ...
        serialized.Should().NotContain("user@example.test").And.NotContain("SECRET_CODE").And.NotContain("empty-button");
        serialized.ToLowerInvariant().Should().NotContain("authorization").And.NotContain("storagestate");
    }
